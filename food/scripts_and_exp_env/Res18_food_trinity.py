...
if __name__ == '__main__':
    ...
    if config['running_mod'] == 'eval':
        ...
        plot_confusion_matrix(cm,
                              ...
                              save_path=f"Plots/confusion_matrix_food101_ResNet_{model_name}_{config['erase_ratio'] * 100}.png",
                              normalize=True)
    ...
    while epoch <= config['n_epochs']:
        ...
        if avg_acc > max_acc:
            ...
            # 根据运行模式决定保存的模型名称
            if config['running_mod'] == 'finetune':
                save_path = 'models/resnet18_finetuned.pth'
            ...
            # 根据运行模式决定备份的模型名称
            if config['running_mod'] == 'finetune':
                backup_path = 'models/resnet18_finetuned_backup.pth'
            else:
                backup_path = 'models/resnet18_backup.pth'
        ...
        if config['constant_save'] and epoch % 5 == 0:
            torch.save(resnet.state_dict(), f'models/finetune/resnet_trinity_{epoch}.pth')
        epoch += 1
    writer.close()
