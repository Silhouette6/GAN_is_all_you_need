...
# The file has been changed into an English version.
...
vit_config = {'num_workers': 8,
              ...
              'label_smoothing': 0.1,
              'mix_ratio': 0.5964,
              'weight_decay': 1e-5,  # 于Vit_optuna_v2中被添加为搜索的参数之一
              }
...
def check_training_stagnation(acc_window, mode, res_extra_threshold, vit_extra_threshold):
    """
    检查训练是否停滞，并决定是否切换训练对象
    
    参数:
    epoch: 当前训练轮次
    acc_history: 精度历史记录字典，包含'res'和'vit'两个键
    model_name: 当前训练的模型名称 ('res' 或 'vit')
    config: 配置字典
    ...
    config: 更新后的配置字典
    """
    # 确保acc_history中有足够的数据
    # if epoch <= 10 or len(acc_history) < 10:
    # 计算最近10轮的精度增长率
    # recent_accs = acc_window[-10:]
    growth = acc_window[-1] - acc_window[0]
    
    # 判断增长率是否过低（可以根据需要调整阈值）
    stagnation_threshold = 0.065  # 1%的增长被视为停滞
    is_stagnant = growth < stagnation_threshold
    
    # 如果判定为停滞，切换训练对象并调整extra_threshold
    if is_stagnant:
        # 确定另一个模型的名称
        other_model = 'train_vit' if mode == 'train_res' else 'train_res'

        if mode == 'train_res':
            res_extra_threshold = 0
            vit_extra_threshold += 0.06
            print(f"detected {mode} stagnante, switch to {other_model}, and modified extra-threshold")
            mode = 'train_vit'
            return res_extra_threshold, vit_extra_threshold, mode
        else:
            res_extra_threshold += 0.06
            vit_extra_threshold = 0
            print(f"detected {mode} stagnante, switch to {other_model}, and modified extra-threshold")
            mode = 'train_res'
            return res_extra_threshold, vit_extra_threshold, mode

    else:
        print(f"{mode} > 10 epochs but not stagnante, continue.")
        return None
...
if __name__ == '__main__':
    ...
    optimizer_res = optim.AdamW(resnet.parameters(), lr=res_config['lr'], weight_decay=1e-5)
    ...
    while epoch <= config['n_epochs']:
        ...
        if mode == 'train_res':
            res_acc_window.append(res_total_val_acc / len(val_loader))
            vit_acc_window.append('114514')
            if len(res_acc_window) > 0 and res_acc_window[-1] > (vit_total_val_acc / len(val_loader)) + config['threshold'] + res_extra_threshold:
                mode = 'train_vit'
            elif '114514' not in res_acc_window and len(res_acc_window) == window_size:
                if check_training_stagnation(res_acc_window, mode, res_extra_threshold, vit_extra_threshold) is not None:
                    res_extra_threshold, vit_extra_threshold, mode = check_training_stagnation(res_acc_window, mode, res_extra_threshold, vit_extra_threshold)
                    res_acc_window.clear()
                    vit_acc_window.clear()

        elif mode == 'train_vit':
            vit_acc_window.append(vit_total_val_acc / len(val_loader))
            res_acc_window.append('114514')
            if len(res_acc_window) > 0 and vit_acc_window[-1] > (res_total_val_acc / len(val_loader)) + config['threshold'] + vit_extra_threshold:
                mode = 'train_res'
            elif '114514' not in vit_acc_window and len(vit_acc_window) == window_size:
                if check_training_stagnation(vit_acc_window, mode, res_extra_threshold, vit_extra_threshold) is not None:
                    res_extra_threshold, vit_extra_threshold, mode = check_training_stagnation(vit_acc_window, mode, res_extra_threshold, vit_extra_threshold)
                    vit_acc_window.clear()
                    res_acc_window.clear()
        ...
        if avg_res_acc > max_res_acc:
            ...
            torch.save(resnet.state_dict(), 'models/v2_0/resnet_mutual_learning.pth')
            print(f'Res model has been saved, val_acc={avg_res_acc * 100:.2f}%, now epoch: {epoch}')
        if epoch == config['n_epochs']:
            torch.save(resnet.state_dict(), 'models/v2_0/res_mutual_learning_backup.pth')
        ...
        if avg_vit_acc > max_vit_acc:
            ...
            torch.save(vit.state_dict(), 'models/v2_0/vit_mutual_learning.pth')
            print(f'ViT model has been saved, val_acc={avg_vit_acc * 100:.2f}%, now epoch: {epoch}')
        if epoch == config['n_epochs']:
            torch.save(vit.state_dict(), 'models/v2_0/vit_mutual_learning_backup.pth')
            print('Last training result has been backed up!')
        print('---------------------------------------------------------------------------------------------')
        # ---记录lr---
        ...
        scheduler_vit.step()
        if epoch % 5 == 0:
            torch.save(resnet.state_dict(), f'models/v2_0/resnet_mutual_learning_{epoch}.pth')
            torch.save(vit.state_dict(),f'models/v2_0/vit_mutual_learning_{epoch}.pth')
        epoch += 1
