...
class ResNet18(nn.Module):  # 构建一个类ResNet18架构的残差神经网络
    def __init__(self, dropout=res_config['dropout'], num_classes=102):
        super().__init__()
...
class ViT(nn.Module):
    def __init__(self, embed_dim=vit_config['embed_dim'], depth=vit_config['depth'], num_classes=102):
        super().__init__()
...
if __name__ == '__main__':
    ...
    while epoch <= config['n_epochs']:
        ...
        if avg_res_acc > max_res_acc:
            ...
            torch.save(resnet.state_dict(), 'models/resnet_flower_mutual_learning.pth')
            print(f'Res model has been saved, val_acc={avg_res_acc * 100:.2f}%, now epoch: {epoch}')
        if epoch == config['n_epochs']:
            torch.save(resnet.state_dict(), 'models/res_mutual_flower_learning_backup.pth')
        ...
        if avg_vit_acc > max_vit_acc:
            ...
            torch.save(vit.state_dict(), 'models/vit_flower_mutual_learning.pth')
            print(f'ViT model has been saved, val_acc={avg_vit_acc * 100:.2f}%, now epoch: {epoch}')
        if epoch == config['n_epochs']:
            torch.save(vit.state_dict(), 'models/vit_flower_mutual_learning_backup.pth')
            print('Last training result has been backed up!')
        # ---记录lr---
