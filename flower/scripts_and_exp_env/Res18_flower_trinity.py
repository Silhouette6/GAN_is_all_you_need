...
config = {'n_epochs': -1,
          ...
          'lr_decay_epochs': 30,  # normal:25 finetune:15
          'dropout': 0.10092,
          'img_size': 224,
          ...
          'constant_save': True,
          'model_path': 'models/saved/resnet_flower_ml6.pth',
          }
...
def get_dataloader(root, transform_train, transform_val, batch_size):
    ...
    config['class_names'] = [str(i) for i in range(102)]
    
    train_loader = DataLoader(combined_train, batch_size=batch_size, shuffle=True,
                              drop_last=False, num_workers=config['num_workers'], pin_memory=True)
...
if __name__ == '__main__':
    ...
    if config['running_mod'] in ['finetune', 'eval']:
        ...
        try:
            resnet.load_state_dict(torch.load(pretrained_path, map_location='cpu'))
            resnet.to(device)
    ...
    if config['running_mod'] == 'eval':
        ...
        # print(type(all_targets),type(all_preds))
        # print("y_true 中每个类出现次数：")
        # print(Counter(all_targets))  # 如果某些类别根本没有出现，说明测试集中没有这些类

        # print("y_pred 中每个类被预测的次数：")
        # print(Counter(all_preds))  # 如果某些类从未被预测出来，也说明模型忽视了这些类
        # 生成Confusion Matrix
        cm = confusion_matrix(all_targets, all_preds)
        plot_confusion_matrix(cm,
                              class_names=config['class_names'],
                              save_path=f"Plots/confusion_matrix_flower102_ResNet_{config['erase_ratio'] * 100}.png",
                              normalize=True)
    ...
    while epoch <= config['n_epochs']:
        ...
        if avg_acc > max_acc:
            ...
            # 根据运行模式决定保存的模型名称
            if config['running_mod'] == 'finetune':
                save_path = 'models/resnet18_flower_finetuned.pth'
                print(f'Fine-tuned model saved, Val_acc={avg_acc * 100:.2f}%, current epoch: {epoch}')
            else:
                save_path = 'models/resnet18_flower.pth'
                print(f'Model saved, Val_acc={avg_acc * 100:.2f}%, current epoch: {epoch}')
            torch.save(resnet.state_dict(), save_path)
        elif epoch == config['n_epochs']:
            # 根据运行模式决定备份的模型名称
            if config['running_mod'] == 'finetune':
                backup_path = 'models/resnet18_flower_finetuned_backup.pth'
            else:
                backup_path = 'models/resnet18_flower_backup.pth'
        ...
        if config['constant_save'] and epoch % 5 == 0:
            torch.save(resnet.state_dict(), f'models/finetune/resnet_flower_trinity_{epoch}.pth')
        epoch += 1
    writer.close()
