...
config = {'n_epochs': -1,
          ...
          'num_workers': 4,
          'lr': 7.4183e-05,
          'lr_stable_epochs': 50,  # normal:35 finetune:10
...
if __name__ == '__main__':
            ...
            if config['running_mod'] == 'eval':
                if 'pretrain' in config['model_path']:
                    ...
                    vit = timm.create_model(
                        ...
                        num_classes=102  # 替换分类头为Food-101的类别数
                    )
    ...
    if config['running_mod'] == 'eval':
        ...
        plot_confusion_matrix(cm,
                              ...
                              save_path=f"Plots/confusion_matrix_flower102_Vit_{config['erase_ratio'] * 100}.png",
                              normalize=True)
    ...
    while epoch <= config['n_epochs']:
        ...
        if avg_acc > max_acc:
            ...
            # 根据运行模式决定备份的模型名称
            if config['running_mod'] == 'finetune':
                backup_path = 'models/vitc1_finetuned_backup.pth'
            else:
                backup_path = 'models/vitc1_backup.pth'
        ...
        if config['constant_save'] and epoch % 5 == 0:
            torch.save(vit.state_dict(), f'models/finetune/vitc1_trinity_{epoch}.pth')
